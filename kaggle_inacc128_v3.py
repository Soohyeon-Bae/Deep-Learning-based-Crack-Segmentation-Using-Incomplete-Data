...
model = dict(
    ...
    decode_head=dict(
        ...
        align_corners=False,
        loss_decode=dict(
            type='CrossEntropyLoss', class_weight=[0.02, 0.98], use_sigmoid=False, loss_weight=1.0)),
    auxiliary_head=dict(
        type='FCNHead',
        in_channels=1024,
        in_index=2,
...
data = dict(
    ...
    train=dict(
        ...
        dataset=dict(
            type='CustomDataset',
            data_root='dataset/exp1/kaggle_inacc',
            pipeline=train_pipeline,
            split=None,
            img_dir='img',
            ann_dir='mask',
...
log_config = dict(interval=50, hooks=[dict(type='TextLoggerHook'),
                                      dict(type='WandbLoggerHook',
                                           init_kwargs=dict(project='crack_segmentation', name='inaccurate_kaggle_lossweight'))])
work_dir = './work_dirs/crack_segmentation/inaccurate_kaggle_lossweight'